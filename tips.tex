...
\end{lstlisting}
  \item String manipulation: examples \href{https://tldp.org/LDP/abs/html/string-manipulation.html}{here}
    \begin{itemize}
      \item The full list on the \href{https://www.gnu.org/software/bash/manual/html_node/Shell-Parameter-Expansion.html}{official guide}
...
\end{verbatim}
          \item Other arguments:
            \begin{itemize}
              ...
              \item \texttt{env}: a dictionary to be used as system environment.
                For instance, to update to \texttt{PATH}
\begin{lstlisting}[language=python]
import os
...
\end{lstlisting}
              \item \texttt{shell}: whether to run the command through the shell.
                That is to say, with true, \verb|Popen(["ls"])| calls \verb|/bin/sh -c ls| (notice the call to default shell).
                Default is false.
              \item \texttt{stdin}, \texttt{stdout}, \texttt{stderr}: specify the executed program’s standard input, standard output and standard error file handles, respectively.
                Valid values are \texttt{None}, \texttt{PIPE}, \texttt{DEVNULL} (constants defined in the module), an existing file descriptor (a positive integer), and an existing file object with a valid file descriptor.
                With the default settings of \texttt{None}, no redirection will occur.
                \texttt{PIPE} indicates that a new pipe to the child should be created.
                \texttt{DEVNULL} indicates that the special file \texttt{os.devnull} will be used.
                Additionally, \texttt{stderr} can be \texttt{STDOUT}, which indicates that the \texttt{stderr} data from the child process should be captured into the same file handle as for \texttt{stdout}.
                See below for examples.
              \item If \texttt{encoding} or \texttt{errors} are specified, or \texttt{text} (also known as \texttt{universal\_newlines}) is true, the file objects \texttt{stdin}, \texttt{stdout} and \texttt{stderr} will be opened in text mode using the encoding and errors specified in the call or the defaults for \texttt{io.TextIOWrapper}.
            \end{itemize}
...
\end{verbatim}
          \item Mind that, differently from \verb|os.system|, \texttt{subprocess} does not wait for the command to finish, hence the python script continues.
            In order to wait use the \texttt{wait} method.
            Another way to obtain that is to use \texttt{subprocess.run}, see below.
\begin{verbatim}
list_dir = subprocess.Popen(["ls", "-l"])
list_dir.wait()
\end{verbatim}
        \end{itemize}
      \item \texttt{subprocess.run}: similar to \texttt{Popen} but directly run the command and wait for its completion.
        \begin{itemize}
          \item Arguments:
            \begin{itemize}
              \item All those of \texttt{Popen}
              \item \texttt{capture\_stdout}: store the output (better to use this in combination with \texttt{text=True})
\begin{lstlisting}[language=python]
# cmd = [...]
...
\end{lstlisting}
              \item \texttt{check}: whether to check if the command completes. If not, it raises a \texttt{python} exception
            \end{itemize}
        \end{itemize}
    \end{itemize}
  \item \href{https://numpy.org/doc/stable/index.html}{\texttt{numpy}} (In the examples below we suppose it has been loaded as \texttt{np})
    \begin{itemize}
      \item Products:
        \begin{itemize}
          \item An \href{https://mkang32.github.io/python/2020/08/30/numpy-matmul.html}{introduction}
          \item The usual \emph{star} product, ``\texttt{*}'', alias of \texttt{np.multiply}, acts element-wise
\begin{lstlisting}[language=python]
a = numpy.array([1, 2, 3])
b = numpy.array([4, 5, 6])
a * b  # Gives [4, 10, 18]
...
\end{lstlisting}
  \item Variables
    \begin{itemize}
      \item Declared with keyword \mint{Rust}|let|
      \item Can be typed (annotated) or not: \mint{Rust}|let x: u32 = 5;| unsigned 32-bit integer
...
\end{minted}
    ...
    \end{itemize}
  \item Constants: defined with \mint{Rust}|const|
    \begin{itemize}
